<form method="post" action="{{route('treatments.services.addToCart')}}">
   ...
   <div class="cart-single-item m-t-30">
      <div>
         ...
         <h4 class="col-blue m-0" id="totalAmountTrayDisplay"> From: {{empty($service->lowestPrice) || $service->lowestPrice->price > $service->price ? '$'.number_format($service->price) : '$'.number_format($service->lowestPrice->price)}} </h4>
